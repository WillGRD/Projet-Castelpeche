<?php require_once($_SERVER['DOCUMENT_ROOT'] . '/castelpeche/templates/header.php') ?>

<div class="typep">
    <h1>La pêche à la Truite</h1>
    <p>La pêche à la truite est une activité populaire et appréciée par de nombreux pêcheurs.
        Elle se pratique principalement dans les rivières, les ruisseaux et les lacs où les truites sont abondantes.
        La truite est un poisson agile et méfiant, ce qui en fait une proie passionnante à cibler.</p>
</div>
<div class="carna">
    <article class="carnafull">
        <img src="../../img/moulinet1.webp" alt="Photo d'un moulinet">
        <div class="h3p">
            <h3>Nos Moulinets</h3>
            <p>Nos moulinets pour la pêche à la truite sont conçus pour offrir une excellente performance et durabilité.
                Ils sont équipés de systèmes de freinage précis et réactifs,
                ce qui vous permet de contrôler facilement la tension du fil lors des combats avec les truites.
                De plus, ils sont dotés de mécanismes de récupération fluides et fiables, assurant une récupération efficace de la ligne.</p>
        </div>
    </article>
    <article class="carnafull">
        <img src="../../img/pêche6.webp" alt="Photo de deux boites de leurres">
        <div class="h3p">
            <h3>Nos Leurres et Appâts</h3>
            <p>Nos appâts naturels comprennent une variété d'options telles que les vers de terre, les asticots,
                les vers de vase, les pâtes. Ils sont soigneusement sélectionnés pour leur fraîcheur et leur attrait pour les poissons.
                Nous proposons également une gamme d'appâts artificiels tels que les leurres souples, les leurres durs, les cuillères,
                les spinnerbaits et les leurres de surface. Ces leurres sont disponibles dans une multitude de couleurs,
                de formes et de tailles pour imiter les proies des poissons et les inciter à mordre.</p>
        </div>
    </article>
    <article class="carnafull">
        <img src="../../img/canne.webp" alt="Photo d'un lancé">
        <div class="h3p">
            <h3>Nos Cannes & Enssembles</h3>
            <p>Dans notre magasin, vous trouverez une vaste sélection de cannes et d'ensembles de pêche adaptés à tous les niveaux et toutes les techniques.
                Fabriquées avec des matériaux de haute qualité tels que le carbone et la fibre de verre, nos cannes offrent légèreté, résistance et réactivité.
                Que vous pratiquiez la pêche au coup, la pêche à la mouche, la pêche au lancer léger ou la pêche aux leurres, nous avons la canne idéale pour vous.
                De plus, nos ensembles complets facilitent votre choix en incluant canne,
                moulinet et parfois du petit matériel supplémentaire. Avec différentes tailles,
                puissances et actions disponibles, vous trouverez la canne parfaite pour vos besoins,</p>
        </div>
    </article>
    <article class="carnafull">
        <img src="../../img/materielcarna.webp" alt="Photo d'une boite avec du petit matériel">
        <div class="h3p">
            <h3>Petit matériel</h3>
            <p>Dans notre magasin, vous trouverez tout le petit matériel essentiel pour la pêche à la truite.
                Notre sélection comprend des flotteurs, des plombs, des hameçons, des élastiques de ligne,
                des bas de ligne et autres accessoires. Nous travaillons avec les meilleures marques pour vous offrir des produits de qualité.</p>
        </div>
    </article>
</div>

<?php require_once($_SERVER['DOCUMENT_ROOT'] . '/castelpeche/templates/footer.php') ?>